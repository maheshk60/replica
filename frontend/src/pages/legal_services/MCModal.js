...
// import { assignMC, getUsers } from '../../services/api';
...
// export default function MCModal({ job, onClose, onAssigned }) {
//   const { user } = useAuth();
...
//   useEffect(() => {
//     setMakers((job?.makers || []).map(m => String(m.id)));
//     setCheckers((job?.checkers || []).map(c => String(c.id)));
...
//     getUsers()
//       .then(r => {
//         const raw = Array.isArray(r.data) ? r.data : (r.data.users || r.data.results || []);
...
//       };
//       const res = await assignMC(job.id, payload, job.caseType);
//       onAssigned(res.data);
//     } catch (e) {
...
//   const periodLabel = job?.period_label || job?.period_1_label;
...
//                           </div>
//                         </div>
//                         {sel && <span style={{ fontSize: 11, color, flexShrink: 0 }}>✓</span>}
...
const getUsers = () =>
  api.get('employee/employees/', { params: { is_active: true } }).then((res) => {
    ...
    const mapped = raw.map((emp) => ({
      ...
      name: `${emp.first_name || ''} ${emp.last_name || ''}`.trim() || emp.user?.email || 'Unknown',
      username: emp.user?.email || `user${emp.user_id || emp.id}`,
...
// Pass 'income-tax-litigations' as the 3rd argument when calling from the Income Tax page.
