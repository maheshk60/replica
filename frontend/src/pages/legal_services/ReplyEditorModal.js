...
//   // ✅ New States for Supporting Docs
...
//   // ✅ NEW: Helper to upload supporting docs attached to the HTML reply ID
...
//       await uploadNewSupports(saved.data.id); // ✅ Attach supporting files
...
//   const doDeleteAndReload = async (idToDelete) => {
//     // Delete HTML Reply (backend handles cascade delete of pending_support via the custom perform_destroy I gave you)
//     await api.delete(`/legal-services/notice-replies/${idToDelete}/`);
//     const res = await api.get('/legal-services/notice-replies/', { params: { notice: noticeId } });
//     const list = Array.isArray(res.data) ? res.data : (res.data.results || []);
//     setReplyHistory(list);
...
//           <div style={{ display: 'flex', alignItems: 'center', gap: 8, flex: 1, minWidth: 0 }}>
//             <span style={{ color: '#fff', flexShrink: 0 }}><FileIcon /></span>
//             <input value={title} onChange={(e) => setTitle(e.target.value)} disabled={!canEditContent} placeholder="Enter file name..." style={{ background: 'rgba(255,255,255,0.08)', border: '1px solid rgba(255,255,255,0.15)', borderRadius: 4, padding: '3px 8px', color: '#fff', fontSize: 11, fontWeight: 600, outline: 'none', width: 180, fontFamily: 'inherit' }} />
...
//             {/* ✅ NEW: 📎 Button to open Attachments modal */}
//             {canEditContent && (
//               <HdrBtn title="Supporting Docs" onClick={() => setShowSupportModal(true)} badge={(existingSupports.length + newSupportFiles.length) || null}>📎</HdrBtn>
//             )}
...
//       {showSupportModal && (
//         <div onClick={() => setShowSupportModal(false)} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 9999999 }}>
//           <div onClick={e => e.stopPropagation()} style={{ background: '#fff', borderRadius: 10, width: 450, maxWidth: '90%', boxShadow: '0 20px 60px rgba(0,0,0,0.3)', overflow: 'hidden' }}>
//             <div style={{ padding: '12px 18px', background: C.navy, color: '#fff', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
//               <h3 style={{ margin: 0, fontSize: 14, fontWeight: 700 }}>📎 Attach Supporting Documents</h3>
//               <button onClick={() => setShowSupportModal(false)} style={{ background: 'none', border: 'none', color: '#fff', cursor: 'pointer', fontSize: 14 }}>✕</button>
//             </div>
//             <div style={{ padding: 20, maxHeight: '60vh', overflowY: 'auto' }}>
//               {existingSupports.length > 0 && (
//                 <div style={{ marginBottom: 16 }}>
//                   <div style={{ fontSize: 11, fontWeight: 700, color: C.slate, marginBottom: 8, textTransform: 'uppercase' }}>Uploaded ({existingSupports.length})</div>
//                   {existingSupports.map(doc => (
//                     <div key={doc.id} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '8px 10px', background: '#f8fafc', borderRadius: 6, marginBottom: 6 }}>
//                       <a href={doc.file_url} target="_blank" rel="noreferrer" style={{ fontSize: 12, color: C.blue, textDecoration: 'none', flex: 1, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>📎 {doc.file_name}</a>
//                       {['draft', 'rejected'].includes(doc.review_status) && <button onClick={async () => { if (!window.confirm('Delete this file?')) return; try { await api.delete(`/legal-services/notice-documents/${doc.id}/`); setExistingSupports(existingSupports.filter(d => d.id !== doc.id)); showToast('🗑 Deleted'); } catch { showToast('⚠ Cannot delete'); } }} style={{ background: 'none', border: 'none', color: C.red, cursor: 'pointer', marginLeft: 6 }}>✕</button>}
//                     </div>
//                   ))}
//                 </div>
//               )}
...
//                 <div style={{ marginBottom: 16 }}>
//                   <div style={{ fontSize: 11, fontWeight: 700, color: C.amber, marginBottom: 8, textTransform: 'uppercase' }}>To Upload (on Save)</div>
//                   {newSupportFiles.map((file, i) => (
//                     <div key={i} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '8px 10px', background: C.amberBg, borderRadius: 6, marginBottom: 6, border: `1px dashed ${C.amberBorder}` }}>
//                       <span style={{ fontSize: 12, color: C.ink, flex: 1, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>📎 {file.name}</span>
//                       <button onClick={() => setNewSupportFiles(newSupportFiles.filter((_, idx) => idx !== i))} style={{ background: 'none', border: 'none', color: C.red, cursor: 'pointer', marginLeft: 6 }}>✕</button>
//                     </div>
//                   ))}
//                 </div>
//               )}
//               {existingSupports.length === 0 && newSupportFiles.length === 0 && <div style={{ padding: 30, textAlign: 'center', color: C.muted, fontSize: 12, fontStyle: 'italic' }}>No supporting documents attached to this reply.</div>}
//               <input type="file" multiple accept=".pdf,.doc,.docx,.jpg,.png,.xlsx,.xls" onChange={(e) => setNewSupportFiles([...newSupportFiles, ...Array.from(e.target.files)])} style={{ display: 'block', marginTop: 10, fontSize: 12 }} />
//               <div style={{ fontSize: 10, color: C.muted, marginTop: 4 }}>Files will be uploaded when you Save Draft or Send for Review.</div>
//             </div>
//             <div style={{ padding: '10px 18px', borderTop: '1px solid #eee', background: '#f8fafc', display: 'flex', justifyContent: 'flex-end' }}>
//               <button onClick={() => setShowSupportModal(false)} style={{ padding: '6px 16px', border: '1px solid #ddd', borderRadius: 5, background: '#fff', cursor: 'pointer', fontSize: 12, fontWeight: 600 }}>Close</button>
//             </div>
//           </div>
//         </div>
//       )}
