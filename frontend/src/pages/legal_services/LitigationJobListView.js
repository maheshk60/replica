...
//   const STS = ['All', 'wip','attention_required', 'open', 'closed'];
...
export default function LitigationJobListView({
  ...
  // Sub-service options (same for both — union of all)
  const subServiceOptions = ['All', ...new Set([
  ...
  const goToNotice = (n) => {
    // Build URL with notice deep-link
    const params = new URLSearchParams();
    params.set('type', litigationType);
    if (n.job_id) params.set('caseId', n.job_id);
    params.set('tab', 'activity');
    params.set('notice', n.id);
    navigate(`/legal-services/clients/${n.client_id}?${params.toString()}`);
  };
