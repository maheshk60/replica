...
// import { clientLiteApi, incomeTaxLitigationApi, api } from '../../../services/api';
...
// export default function IncomeTaxCaseModal({ open, onClose, onCreated, editCase }) {
...
//     try {
//       if (editCase) {
//         await incomeTaxLitigationApi.update(editCase.id, payload);
//       } else {
//         await incomeTaxLitigationApi.create(payload);
//       }
//       onCreated();
//       setForm(EMPTY_FORM);
//       onClose();
...
//     <>
//       <style>{`
//         @keyframes itcmFadeIn {
//           from { opacity: 0; }
//           to   { opacity: 1; }
//         }
//         @keyframes itcmSlideUp {
...
//               {editCase ? 'Edit' : 'New'} Income Tax Litigation
//             </h3>
//             <button 
...
//               {assignOpen && (
//                 <div className="mt-1 max-h-[180px] overflow-y-auto border border-gray-200 rounded-lg bg-white shadow-[0_8px_20px_rgba(20,20,40,0.1)] z-10 relative">
...
//               <button 
//                 type="submit" 
//                 className="px-5 py-2 rounded-lg border-none bg-gradient-to-br from-[#14b8a6] to-[#2dd4bf] text-[#0a1628] font-bold text-[13px] cursor-pointer disabled:opacity-60 disabled:cursor-not-allowed transition-opacity"
//                 disabled={submitting}
//               >
//                 {submitting
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      .then(([clientsRes, subRes, employeesRes]) => {
        ...
        setSubServices(allSubs.filter((s) => s.job_category === 'income-tax'));
        setEmployees(employeesRes.data.results || employeesRes.data);
      })
      .catch(() => setError('Failed to load form options.'))
      .finally(() => setLoadingOptions(false));
