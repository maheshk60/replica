...
//       // Only summary reviews from ReviewRequest
//       setReviews(reviewData.filter(r => r.action_type === 'summary' || r.action_type === 'notice_edit'));

//       // Only notices that have review status set
...
//         // Exclude drafts — only show replies that have been sent for review
...
//   // ══════════════════════════════════════════════════════════════
//   // SUMMARY REVIEW ACTIONS (from old ReviewTab logic)
//   // ══════════════════════════════════════════════════════════════
//   const canApproveSummary = (review) => {
...
//   const canRejectSummary = (review) => canApproveSummary(review);

//   const handleApproveSummary = (review) => {
//     // Escalated reviews → direct approve (no modal, Founder only)
...
//   const handleRejectSummary = (review) => setRejectTarget(review);
...
//   const allItems = [...summaryReviewItems, ...noticeEditReviewItems, ...noticeReviewItems, ...replyReviewItems]
...
//                   canApprove={canApproveSummary(item)}
//                   canReject={canRejectSummary(item)}
//                   onApprove={() => handleApproveSummary({ ...item, id: item._originalId })}
//                   onReject={() => handleRejectSummary({ ...item, id: item._originalId })}
//                 />
//               );
//             }
...
//                   review={item}
//                   busy={busyId === item._originalId}
//                   canApprove={canApproveSummary(item)}  // Same permission logic
//                   canReject={canRejectSummary(item)}
//                   onApprove={() => handleApproveSummary({ ...item, id: item._originalId })}
//                   onReject={() => handleRejectSummary({ ...item, id: item._originalId })}
//                 />
...
//       {/* ══ MODALS ══ */}
//       {/* Summary review modals — CENTER position */}
//       {rejectTarget && (
//         <RejectModal
//           review={rejectTarget}
...
//           onAccepted={doAccept}
//           hideEscalate={!isFounder ? false : false}
//           // NOTE: We pass a prop `hideEscalate` — you'll need to add support in AcceptModal
//           // OR we handle it here based on user role. Keeping simple: Founder sees checkbox.
//         />
//       )}

//       {/* Notice detail modal — for checker to view + act */}
//       {selectedNoticeId && (
//         <NoticeDetailModal
...
//       {/* Reply editor modal — for checker to view + act */}
//       {replyModalData && (
//         <ReplyEditorModal
...
//         marginBottom: review.status === 'rejected' ? 8 : (canApprove || canReject) ? 10 : 0,
//       }}>
...
//         <div style={{
//           marginBottom: (canApprove || canReject) ? 10 : 0, padding: '9px 12px',
...
// // ══════════════════════════════════════════════════════════════════
// // NOTICE EDIT REVIEW CARD — shows old → new changes
// // ══════════════════════════════════════════════════════════════════
...
//         padding: '10px 12px', background: C.bgSoft,
//         border: `1px solid ${C.borderLight}`, borderRadius: 7,
//         marginBottom: item.status === 'rejected' ? 8 : 10,
//       }}>
//         <div style={{ display: 'flex', flexDirection: 'column', gap: 4, fontSize: 12 }}>
//           {item.din_number && <PayloadRow label="DIN" value={item.din_number} />}
//           {item.officer && <PayloadRow label="Officer" value={item.officer} />}
//           {item.section && <PayloadRow label="Section" value={item.section} />}
//           {item.due_date && <PayloadRow label="Due Date" value={fmtDate(item.due_date)} />}
//           {item.extended_due_date && <PayloadRow label="Extended Due" value={fmtDate(item.extended_due_date)} />}
//           <PayloadRow
//             label="Documents"
//             value={
//               item.status === 'approved'
//                 ? `${item.reply_docs_count} in Replies`
//                 : `${item.pending_docs_count} to review`
//             }
//           />
//         </div>
//       </div>
...
//         {item.notice?.due_date && <InlineField label="Notice Due" value={fmtDate(item.notice.due_date)} />}
//       </div>
...
//       </span>
//       <span style={{
//         fontSize: 11.5, color: C.ink, fontWeight: 600,
//       }}>
//         {value}
//       </span>
//     </div>
