...
//   // ✅ STRICT SPLIT: Pending/Escalated stay in Active. 
//   // We also keep the MOST RECENT Approved/Rejected item in Active so the Maker can easily see its status.
//   const { activeItems, historyItems } = useMemo(() => {
//     const active = [], completed = [];
//     allItems.forEach(item => { 
//       if (item.status === 'pending' || item.status === 'escalated') active.push(item); 
...
//     // ✅ Move the single most recent completed item into the Active list
//     if (completed.length > 0) {
//       active.push(completed[0]);
//       // The rest go to History
