...
export default function LitigationDashboard({
  ...
  // ✅ Fetch notices for this litigation type
  ...
  // ✅ Compute notice attention status (frontend-side)
  ...
  // ✅ Shared helper: is this case in the current user's inbox?
  const isMyTurn = (c) => {
    const userId = user?.id;
    const isCeoRole = user?.role === 'Admin' || user?.role === 'Founder';
    const review = reviewsByJob[c.id];
  ...
  // ✅ My Pending Task — WIP cases only
  const myPendingTasks = useMemo(() => {
    return cases.filter((c) => {
      const status = getEffectiveStatus(c);
  ...
  // ✅ My Attention Required — urgent cases assigned to current user
  const myAttentionTasks = useMemo(() => {
    return cases.filter((c) => {
      const status = getEffectiveStatus(c);
      if (status !== 'attention_required') return false;
      return isMyTurn(c);
    });
    // eslint-disable-next-line
  }, [cases, reviewsByJob, user]);
  ...
  useEffect(() => {
    setLoading(true); setError(null);
    Promise.all([fetchCases(), fetchReviews(), fetchNotices()])
      .finally(() => setLoading(false));
    // eslint-disable-next-line
  }, []);
  ...
  // ✅ Navigate directly to activity tab for attention cases
  const goToClientActivity = (c) => {
    const basePath = typeof clientPath === 'function' ? clientPath(c) : `/legal-services/clients/${c.client}`;
  ...
  const attentionNotices = notices.filter(n => isNoticeAttention(n)).length;

  // ── Cases Cards Config ──
  const CASE_CARDS = [
    {
      label: 'All Cases', value: totalJobs, color: P, icon: '📊',
      sub: 'all cases', onClick: () => goToFilteredCases(),
    },
    {
      label: 'WIP', value: wipJobs, color: '#f59e0b', icon: '🔄',
      sub: 'work in progress', onClick: () => goToFilteredCases('wip'),
    },
    {
      label: 'Open', value: openJobs, color: '#0ea5e9', icon: '📂',
      sub: 'all notices acknowledged', onClick: () => goToFilteredCases('open'),
    },
    {
      label: 'Closed', value: closedJobs, color: GR, icon: '✅',
      sub: 'completed cases', onClick: () => goToFilteredCases('closed'),
    },
    {
      label: 'Attention Required', value: attentionJobs.length,
      color: attentionJobs.length > 0 ? RED : MUT, icon: '⏰',
      sub: 'needs immediate action', onClick: () => goToFilteredCases('attention_required'),
    },
  ];
  ...
  const NOTICE_CARDS = [
    {
      label: 'All Notices', value: totalNotices, color: P, icon: '🔔',
      sub: 'all notices across cases', onClick: () => goToFilteredNotices(),
    },
    {
      label: 'WIP', value: wipNotices, color: '#f59e0b', icon: '🔄',
      sub: 'awaiting maker work', onClick: () => goToFilteredNotices('wip'),
    },
    {
      label: 'Under Review', value: underReviewNotices, color: PURPLE, icon: '👁',
      sub: 'pending checker review', onClick: () => goToFilteredNotices('under_review'),
    },
    {
      label: 'Open', value: openNotices, color: '#0ea5e9', icon: '📂',
      sub: 'acknowledged', onClick: () => goToFilteredNotices('open'),
    },
    {
      label: 'Attention Required', value: attentionNotices,
      color: attentionNotices > 0 ? RED : MUT, icon: '⏰',
      sub: 'due ≤ 5 days or overdue', onClick: () => goToFilteredNotices('attention_required'),
    },
  ];
  ...
  return (
    <div style={{ padding: '2px 14px 32px', minHeight: '100%', boxSizing: 'border-box', background: '#f8f9fc' }}>
      ...
      {/* ═══════════ BOTTOM PANELS — 3 cards (Recent Jobs removed) ═══════════ */}
      <div style={{
        display: 'grid',
        gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))',
        gap: 16,
      }}>
        {/* ── 1. My Pending Task (WIP cases assigned to me based on review state) ── */}
        <div style={{
          background: WH, border: `1.5px solid ${BRD}`, borderRadius: 16,
          padding: 22, display: 'flex', flexDirection: 'column', height: 380,
        }}>
          <div style={{
            display: 'flex', justifyContent: 'space-between', alignItems: 'center',
            marginBottom: 14, flexShrink: 0,
            ...
            <div>
              <div style={{
                ...
                letterSpacing: '.08em', color: '#1C1E2E',
              }}>
                My Pending Task
              </div>
              <div style={{ fontSize: 10.5, color: MUT, marginTop: 3, fontWeight: 500 }}>
                WIP cases waiting for you
              </div>
            </div>
            {myPendingTasks.length > 0 && (
              <span style={{
                background: '#f59e0b', color: WH, borderRadius: 99,
        ...
        {/* ── 2. Attention Required (my urgent cases → directly to activity tab) ── */}
        <div style={{
          background: WH,
          border: `1.5px solid ${myAttentionTasks.length > 0 ? '#fecaca' : BRD}`,
          borderRadius: 16,
          padding: 22, display: 'flex', flexDirection: 'column', height: 380,
          boxShadow: myAttentionTasks.length > 0 ? '0 0 0 3px rgba(220,38,38,0.05)' : 'none',
        }}>
          <div style={{
            display: 'flex', justifyContent: 'space-between', alignItems: 'center',
            marginBottom: 14, flexShrink: 0,
            ...
            <div>
              <div style={{
                fontSize: 12, fontWeight: 700, textTransform: 'uppercase',
                letterSpacing: '.08em',
                color: myAttentionTasks.length > 0 ? RED : '#1C1E2E',
            ...
            {myAttentionTasks.length > 0 && (
              <span style={{
                ...
                {myAttentionTasks.length}
              </span>
            )}
          </div>
          <div style={{ overflowY: 'auto', flex: 1 }}>
            <AttentionPanel
              jobs={myAttentionTasks}
              onJobClick={(job) => goToClientActivity(job)}
            />
          </div>
        </div>
