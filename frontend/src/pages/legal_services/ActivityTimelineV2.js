...
function getDaysInfo(dueDate) {
  if (!dueDate) return null;
  const today = new Date(); today.setHours(0, 0, 0, 0);
  const due = new Date(dueDate); due.setHours(0, 0, 0, 0);
  const diff = Math.ceil((due - today) / (1000 * 60 * 60 * 24));
  if (diff < 0) return { text: `${Math.abs(diff)}d overdue`, color: C.red, urgent: true };
  if (diff === 0) return { text: 'Due today', color: C.red, urgent: true };
  if (diff <= 3) return { text: `${diff}d left`, color: C.red, urgent: true };
...
function SidebarNoticeRow({ notice, isSelected, onClick }) {
  ...
  const days = getDaysInfo(notice.extended_due_date || notice.due_date);
  ...
  return (
    <div
      ...
      style={{
        padding: '8px 12px',
        cursor: 'pointer',
        borderLeft: `2.5px solid ${isSelected ? C.navy : 'transparent'}`,
        background: isSelected ? C.blueBg : (hover ? C.bgSoft : '#fff'),
...
function NoticeDetailView({
  ...
  const ackLocked = !hasCourtNotice || !hasApprovedReply;  // Both conditions
  ...
  const canDeleteRejectedAck = isAssignedMaker && ackReviewStatus === 'rejected';
  const canDeleteFiles = isFounder;
...
export default function ActivityTimelineV2({
  ...
  const canAddNotice = isAssignedMaker;
          ...
          <div style={{
            ...
            {notices.length > 0 && (
              <>
                ...
                <StatusCountChip status="closed" count={statusCounts.closed} />
              </>
            )}
