...
//   const handleDownloadReplyAsPdf = (reply) => {
...
//     message.info({ content: 'Opening print dialog...', key: 'pdf-gen', duration: 2 });
...
//                 <RepliesCard
//                   replies={allApprovedReplies}
//                   pendingDocs={notice.pending_docs}
//                   isAssignedMaker={isMakerUploader}
//                   locked={!courtNoticeDoc}
//                   deletingId={deletingId}
//                   onReply={onReply}
//                   onUploadReplyClick={() => setShowUploadReplyModal(true)} // ✅ TRIGGERS NEW MODAL
...
// // ══════════════════════════════════════════════════════════════════
// // CLOSE CASE MODAL
// // 3 mandatory uploads: Order, Demand Notice, Computation Sheet
// // Bundle-based review: all 3 approved/rejected together
// // ══════════════════════════════════════════════════════════════════
...
//       await fetchCase();
//       // ✅ Silent update — don't trigger parent refresh (which was closing modal)
//       // Just call onUpdated later when closing OR after submit
//     } catch (e) {
//       message.error(e?.response?.data?.error || 'Failed to upload');
//     } finally {
//       setUploadingType(null);
//     }
//     return false;  // ✅ Return false so antd Upload doesn't handle it
//   };
...
//       zIndex: 3000,  // ✅ Higher than the CloseCaseModal so confirm shows on top
//       onOk: async () => {
...
//           await fetchCase();
//           // ✅ Silent update — don't trigger parent refresh
//         } catch (e) {
//           message.error(e?.response?.data?.error || 'Failed to delete');
//         } finally {
//           setDeletingId(null);
...
//       okText: 'Submit', okType: 'primary', cancelText: 'Cancel', centered: true,
//       zIndex: 3000,  // ✅ Show above CloseCaseModal
//       onOk: async () => {
//         setSubmitting(true);
//         try {
...
//           await fetchCase();
//           onUpdated && onUpdated();  // ✅ Notify parent after submit (this is intentional)
//         } catch (e) {
...
//           </div>
//           <button onClick={() => {
//             onUpdated && onUpdated();  // ✅ Refresh parent when closing modal
//             onClose();
//           }} style={{
...
//               <div style={{ flex: 1 }}>
//                 <div style={{ fontSize: 12.5, fontWeight: 700, color: C.green }}>
//                   Case Closed Successfully
//                 </div>
//                 <div style={{ fontSize: 11, color: C.slate, marginTop: 2 }}>
//                   Approved by <b>{caseData.closure_reviewed_by_name}</b> on {fmtDateTime(caseData.closure_reviewed_at)}
//                 </div>
//               </div>
//             </div>
//           )}
...
//                     <div style={{
//                       padding: '8px 10px', background: '#fff', borderRadius: 5,
//                       border: `1px solid ${C.redBorder}`,
//                       fontSize: 12, color: C.ink, lineHeight: 1.5, marginTop: 6,
//                     }}>
//                       <b>Reason:</b> {caseData.closure_review_note}
//                     </div>
//                   )}
//                   <div style={{ fontSize: 11, color: C.slate, marginTop: 6 }}>
//                     Please upload all 3 documents again to resubmit.
//                   </div>
//                 </div>
//               </div>
//             </div>
...
//             <div style={{
//               padding: '12px 14px', marginBottom: 14,
//               background: C.amberBg, border: `1px solid ${C.amberBorder}`, borderRadius: 8,
//               display: 'flex', alignItems: 'center', gap: 10,
//             }}>
//               <span style={{ fontSize: 18 }}>⏳</span>
//               <div style={{ flex: 1 }}>
...
//                 </div>
//                 <div style={{ fontSize: 11, color: C.slate, marginTop: 2 }}>
//                   Submitted on {fmtDateTime(caseData.closure_submitted_at)}. Documents cannot be modified.
//                 </div>
//               </div>
//             </div>
//           )}
...
//               <span style={{ fontSize: 18 }}>↑</span>
//               <div>
//                 <div style={{ fontSize: 12.5, fontWeight: 700, color: C.purple }}>
//                   Escalated to CEO
//                 </div>
//                 <div style={{ fontSize: 11, color: C.slate, marginTop: 2 }}>
//                   Waiting for CEO's final decision.
//                 </div>
//               </div>
//             </div>
//           )}
...
//         {/* FOOTER */}
//         <div style={{
//           padding: '12px 20px', borderTop: `1px solid ${C.borderLight}`, background: '#fff',
...
//             <Button onClick={() => {
//               onUpdated && onUpdated();  // ✅ Refresh parent when closing
//               onClose();
//             }} size="middle">Close</Button>
//             {canSubmit && (
//               <Button
//                 type="primary"
//                 loading={submitting}
...
// /**
//  * Single closure doc upload card
//  */
...
//     <div style={{
//       background: '#fff',
//       border: `1px solid ${doc ? C.greenBorder : C.border}`,
//       borderRadius: 8,
//       padding: '12px 14px',
...
//               color: hover ? C.blue : C.slate,
//               fontSize: 11, fontWeight: 600,
//               transition: 'color 0.15s',
//             }}
//           >
//             <FileTextOutlined /> <span style={{
...
// // ── REPLIES CARD (Handles HTML, Docs, Upload & Versions) ──
// function RepliesCard({ replies, pendingDocs, isAssignedMaker, locked, deletingId, onReply, onUploadReplyClick, onOpenHtml, onDownloadHtml, onOpenDoc, onDownloadDoc, onDeleteDoc, onDeleteHtml }) {
//   const hasItems = replies.length > 0 || (pendingDocs && pendingDocs.length > 0);

//   return (
//     <div style={{ background: '#fff', border: `1px solid ${C.borderLight}`, borderRadius: 8, display: 'flex', flexDirection: 'column', opacity: locked ? 0.6 : 1 }}>
...
//               Replies
//               {replies.length > 0 && <span style={{ background: C.green, color: '#fff', fontSize: 9, padding: '1px 6px', borderRadius: 99 }}>{replies.length}</span>}
//             </div>
//             <div style={{ fontSize: 9.5, color: C.muted, marginTop: 1 }}>Approved & Pending</div>
//           </div>
//         </div>
        
//         {!locked && isAssignedMaker && (
//           <div style={{ display: 'flex', gap: 6 }}>
//             {/* ✅ OPENS THE NEW BUNDLE MODAL */}
...
//       <div style={{ padding: 8, flex: 1, maxHeight: 180, overflowY: 'auto' }}>
//         {locked ? (
//            <div style={{ padding: '14px 8px', textAlign: 'center', fontSize: 11, color: C.muted, fontStyle: 'italic' }}>Upload court notice to unlock</div>
//         ) : !hasItems ? (
//           <div style={{ padding: '14px 8px', textAlign: 'center', fontSize: 11, color: C.muted, fontStyle: 'italic' }}>No replies yet</div>
//         ) : (
//           <div style={{ display: 'flex', flexDirection: 'column', gap: 5 }}>
//             {pendingDocs && pendingDocs.map(doc => (
//               <ReplyRow key={`pend-${doc.id}`} title={doc.file_name} subtitle={doc.uploaded_by_name} date={doc.uploaded_at} status={doc.review_status}
//                 version={`v${doc.reply_version || '?'}`} isHtml={false} isRejectDeleteAllowed={isAssignedMaker && doc.review_status === 'rejected'} deleting={deletingId === doc.id}
//                 onOpen={() => onOpenDoc(doc)} onDownload={() => onDownloadDoc(doc)} onDelete={() => onDeleteDoc(doc.id)} />
//             ))}
//             {replies.map(reply => (
//               <ReplyRow key={`appr-${reply.id}`} title={reply.title || reply.file_name || 'Untitled'} subtitle={reply.reviewed_by_name} date={reply._sortTime} status="approved"
//                 version={reply._versionLabel} isHtml={reply._isHtml} isRejectDeleteAllowed={false}
//                 onOpen={() => reply._isHtml ? onOpenHtml(reply) : onOpenDoc(reply)}
//                 onDownload={() => reply._isHtml ? onDownloadHtml(reply) : onDownloadDoc(reply)} />
//             ))}
//           </div>
//         )}
//       </div>
...
// // ── SUPPORTING DOCS CARD (Read-only, populated via Reply upload) ──
// function SupportingDocsCard({ docs, approvedReplies, deletingId, onOpen, onDownload, onDelete }) {
//   // ✅ Removed standalone upload button — docs are only uploaded through the Reply Bundle Modal
//   return (
//     <div style={{ background: '#fff', border: `1px solid ${C.borderLight}`, borderRadius: 8, display: 'flex', flexDirection: 'column', opacity: docs.length===0 ? 0.6 : 1 }}>
//       <div style={{ padding: '9px 12px', background: C.amberBg, borderBottom: `1px solid ${C.borderLight}`, display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 8, minHeight: 42 }}>
//         <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
//           <span style={{ fontSize: 14 }}>📎</span>
//           <div>
//             <div style={{ fontSize: 11.5, fontWeight: 800, color: C.amber, letterSpacing: '0.02em' }}>Supporting Docs</div>
//             <div style={{ fontSize: 9.5, color: C.muted, marginTop: 1 }}>Attached to replies</div>
//           </div>
//         </div>
//       </div>
...
//           <div style={{ display: 'flex', flexDirection: 'column', gap: 5 }}>
//             {docs.map(doc => (
//               <ReplyRow key={doc.id} title={doc.file_name} subtitle={doc.uploaded_by_name} date={doc.uploaded_at} status="none"
//                 version={`v${doc.reply_version || '?'}`} isHtml={false} isRejectDeleteAllowed={false} deleting={deletingId === doc.id}
//                 onOpen={() => onOpen(doc)} onDownload={() => onDownload(doc)} onDelete={() => onDelete(doc.id)} />
//             ))}
//           </div>
//         )}
//       </div>
...
// // ══════════════════════════════════════════════════════════════════
// export function UploadReplyBundleModal({ noticeId, nextVersion, onClose, onSuccess }) {
...
//     if (!replyFile) {
//       message.error("Reply document is mandatory.");
//       return;
//     }
//     setUploading(true);
//     try {
...
//       formDataMain.append('reply_version', nextVersion);
//       await api.post('/legal-services/notice-documents/', formDataMain);

//       // 2. Upload Supporting Docs
//       for (const file of supportFiles) {
//         const formDataSup = new FormData();
//         formDataSup.append('notice', noticeId);
//         formDataSup.append('doc_type', 'pending_support');
//         formDataSup.append('file', file);
//         formDataSup.append('reply_version', nextVersion);
//         await api.post('/legal-services/notice-documents/', formDataSup);
//       }

//       message.success("Reply submitted for review!");
//       onSuccess();
//     } catch (e) {
...
//           <h3 style={{ margin: 0, fontSize: 15, fontWeight: 700 }}>Upload Reply (v{nextVersion})</h3>
//           <button onClick={onClose} style={{ background: 'none', border: 'none', color: '#fff', cursor: 'pointer', fontSize: 16 }}>✕</button>
//         </div>
        
//         <div style={{ padding: '20px' }}>
//           <div style={{ marginBottom: 20 }}>
//             <div style={{ fontSize: 12, fontWeight: 700, color: C.slate, marginBottom: 6 }}>1. Reply Document <span style={{ color: C.red }}>*</span></div>
//             <div style={{ fontSize: 11, color: C.muted, marginBottom: 10 }}>Upload the final, signed reply document (PDF or Word).</div>
//             <input type="file" accept=".pdf,.doc,.docx" onChange={(e) => setReplyFile(e.target.files[0])} style={{ width: '100%', fontSize: 13 }} />
//           </div>

//           <div style={{ marginBottom: 20, paddingTop: 20, borderTop: `1px solid ${C.borderLight}` }}>
//             <div style={{ fontSize: 12, fontWeight: 700, color: C.slate, marginBottom: 6 }}>2. Supporting Documents <span style={{ color: C.muted, fontWeight: 400 }}>(Optional)</span></div>
//             <div style={{ fontSize: 11, color: C.muted, marginBottom: 10 }}>Upload any annexures, proofs, or working sheets related to this reply.</div>
...
//               <div style={{ background: C.bgSoft, padding: 10, borderRadius: 6, border: `1px solid ${C.borderLight}` }}>
//                 {supportFiles.map((f, i) => (
//                   <div key={i} style={{ display: 'flex', justifyContent: 'space-between', fontSize: 12, color: C.ink, marginBottom: i === supportFiles.length-1 ? 0 : 6 }}>
...
//           <Button type="primary" onClick={handleSubmit} loading={uploading} style={{ background: C.green, borderColor: C.green, fontWeight: 600 }}>
//             Submit for Review
//           </Button>
//         </div>
//       </div>
